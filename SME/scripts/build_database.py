...
# What is implemented:
# - It attaches all metadata cleanly
# - Implements the required hierarchical chunking (2048/512/128)
# - Uses paragraph chunker for Level-0 (best strategy for compliance PDFs & textbooks)
# - Normalizes & dedupes text
# - Stores chunk_id + parent_id hierarchy
# - Embeds with mpnet-base
# - Supports domain model (commented, plug-in ready)
# - Indexes into Elasticsearch with correct mapping
# - Ready for hybrid retrieval + reranking
# - Ready for LangChain agent integration

# Create the index with the correct dense_vector mapping
# curl -X PUT "localhost:9200/food_safety_rag" \
...
# curl -X DELETE "http://localhost:9200/food_safety_rag"

# ------------------------------------------------------------
# Checks and Verifications
# ------------------------------------------------------------

# Hierarchical chunking verification

# Test A — Count number of level 0, 1, and 2 chunks
# Run scripts/tests/check_levels.py script

# Test B — Inspect a random document
# curl -X GET "localhost:9200/food_safety_rag/_search?size=5" -H 'Content-Type: application/json' -d '{ "query": { "match_all": {} } }' | jq

# Test C — Check token lengths
# print(max(len(chunk['text']) for chunk in to_index))



# Check if embeddings exist
# curl -X GET "localhost:9200/food_safety_rag/_search?size=1" -H "Content-Type: application/json" -d '{ "_source": ["embedding"], "query": { "match_all": {} } }'



# Verify Elasticsearch indexing is correct
# Check the document count
# curl -X GET "localhost:9200/food_safety_rag/_count"

# Sample search to verify data
# curl -X GET "localhost:9200/food_safety_rag/_search?size=3"

# Verify index
# curl -X GET "localhost:9200/food_safety_rag/_search?size=1" -H 'Content-Type: application/json'
# Expect one _source with text, metadata, and embedding fields.

# Check the index mapping (saved)
# curl -X GET "localhost:9200/food_safety_rag/_mapping" | jq

# Metadata mapping (saved)
# curl -X GET "localhost:9200/food_safety_rag/_search?size=1&pretty"